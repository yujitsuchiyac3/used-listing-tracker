...
| CAE Online | `/buy/<カテゴリ>?page=N` | 海外ブローカー型。価格・掲載日・出品者の表示が無いため参考枠扱い |
...
{ "label": "段差計(触針式プロファイラ)",
  ...
  "exclude":  ["attenuator"],
  "exclude_sites": ["caeonline"] }
```

`sites` を書くとそのサイトだけ、`exclude_sites` を書くとそのサイトを除いて判定します。
CAE Online のような「メーカー×型番」単位のカタログ型サイトは、メーカー名だけの
キーワード(例: `cascade microtech`)で数十〜数百件が一致するため、必要に応じて外します。

CAE Online は出品に価格・掲載日・出品者の表示が無く、在庫の鮮度が確認できないため、
実購入向けのグループからは `exclude_sites` で外し、末尾の「【参考】」グループ
(`sites: ["caeonline"]`)に機種名ベースで集めています。
